...
def get_dataloaders(dataset_name, batch_size=Config.BATCH_SIZE, exclude_num=Config.FORGET_SIZE):
    ...
    if dataset_name == 'mnist':
        ...
        # SVHN: 32x32, 3 Channels
        # Chuẩn hóa mean/std đặc thù của bộ dữ liệu số nhà SVHN
        transform_train = transforms.Compose([
            transforms.RandomCrop(32, padding=4),
            transforms.ToTensor(),
        ...
        # SVHN dùng 'split' thay vì 'train'
        full_trainset = torchvision.datasets.SVHN(root=path, split='train', download=True, transform=transform_train)
