...
class MobileNetV2(nn.Module):
    ...
    cfg = [(1,  16, 1, 1),
           (6,  24, 2, 1),  # Giữ nguyên
           (6,  32, 3, 1),  # ĐỔI: stride=1 thay vì 2
           (6,  64, 4, 2),  # Giữ nguyên
           (6,  96, 3, 1),
           (6, 160, 3, 2),
           (6, 320, 1, 1)]
    ...
    def __init__(self, num_channels=3, num_classes=10, img_size=32):
        ...
        self.dropout = nn.Dropout(0.2) # Thêm vào __init__
    ...
    def forward(self, x):
        ...
        # AdaptiveAvgPool2d giúp mô hình linh hoạt với mọi kích thước đầu vào
        # Nó sẽ tự động tính toán pooling để đầu ra luôn là 1x1
        out = F.adaptive_avg_pool2d(out, (1, 1)) 
        
        out = out.view(out.size(0), -1)
        out = self.dropout(out) # Thêm dropout ở đây
        out = self.linear(out)
        return out

    def forward_with_features(self, x):
        # 1. Luồng xử lý qua các lớp tích chập (giữ nguyên)
        out = F.relu(self.bn1(self.conv1(x)))
        out = self.layers(out)
        out = F.relu(self.bn2(self.conv2(out)))
        out = F.adaptive_avg_pool2d(out, (1, 1))
        features = out.view(out.size(0), -1)
        out_for_logits = self.dropout(features)

        logits = self.linear(out_for_logits)
